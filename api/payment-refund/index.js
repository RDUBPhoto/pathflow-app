...
function pickAuthorizeError(responseBody) {
  ...
  if (txErrors.length) {
    ...
    return [code, text].filter(Boolean).join(": ") || "Refund was declined or could not be processed.";
  }
  ...
  return "Refund was declined or could not be processed.";
}
...
module.exports = async function (context, req) {
  ...
  const originalTransactionId = asString(body.originalTransactionId || body.refTransId);
  const accountNumber = maskRefundCard(body.accountNumber || body.cardNumber);
  ...
  if (!provider) {
    context.res = json(400, { ok: false, error: "provider is required." });
    return;
  }
  if (!amount) {
  ...
  if (!originalTransactionId) {
    context.res = json(400, { ok: false, error: "originalTransactionId is required." });
    return;
  }

  if (provider !== "authorize-net") {
    context.res = json(400, {
      ok: false,
      error: `Refund is not configured for provider ${provider}.`
    });
    return;
  }
  ...
  if (!accountNumber) {
    context.res = json(400, {
      ok: false,
      error: "Last 4 card digits are required for Authorize.net refunds."
    });
    return;
  }
  ...
  try {
      ...
      createTransactionRequest: {
        ...
        transactionRequest: {
          ...
          order: {
            ...
            description: `Pathflow invoice refund ${invoiceNumber || invoiceId}`
          }
        }
      }
    };
    ...
    const rawHasTransId =
      ...
      || /"transId"\s*:\s*"?\d+"?/i.test(rawBody);
    const txErrors = Array.isArray(tx.errors) ? tx.errors : [];
    const explicitDecline = (asString(tx.responseCode) && asString(tx.responseCode) !== "1") || txErrors.length > 0;

    const shouldTreatAsApproved =
      !explicitDecline && (
        approved
        || (isOk && !!asString(tx.transId))
        || (rawApproved && (rawHasTransId || isOk))
        || (rawHasTransId && response.status >= 200 && response.status < 300)
      );
    ...
    if (!shouldTreatAsApproved) {
      ...
      context.log.warn("Authorize.net refund declined", {
        tenantId,
        invoiceId,
